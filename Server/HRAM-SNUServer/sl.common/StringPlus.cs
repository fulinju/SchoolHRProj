...
{ 
    ...
    /// 5、GetArrayValueStr(Dictionary<int, int=""></int>  list)得到数组列表以逗号分隔的字符串
    ...
    public class StringPlus
    {
        ...
        public static List<string> GetStrArray(string arraystr)
        {
            return GetStrArray(arraystr, ',', false);
        }
        ...
        public static string ArrayDelOne(string array, string target)
        {
            if (!string.IsNullOrEmpty(array))
            {
                List<string> strArray = GetStrArray(array);
                if (strArray.Contains(target))
                {
                    strArray.Remove(target);
                }
